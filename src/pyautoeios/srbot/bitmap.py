...
def load_image(fname):
    if os.path.exists(fname):
        ...
        return np.asarray(Image.open(os.path.join(BMP_ROOT, fname)))[
            ..., :3
        ]  # strip alpha
...
def cross_correlate(bmp, region):
    ...
    for i in range(3):  # compute each color channel
        ...
        haystack = np.pad(
            haystack, [(0, x + y) for x, y in zip(hsdim, nldim)], "constant"
        )
    ...
    if mode == "xcorr":
                ...
                [
                    [
                        np.sum(
                            val(
                                bmp[mask],
                                region[i : i + hs, j : j + ws][mask],
                                a_rgb=False,
                                b_rgb=False,
                            )[0]
                        )
                        for j in range(wr - ws)
                    ]
                    for i in range(hr - hs)
                ]
            )
        else:
            return np.asarray(
        ...
        if mask is not None:
            return np.asarray(
                [
                    [
                        np.sum(val(bmp[mask], region[i : i + hs, j : j + ws][mask]))
                        for j in range(wr - ws)
                    ]
                    for i in range(hr - hs)
                ]
            ...
            return np.asarray(
                [
                    [
                        np.sum(val(bmp, region[i : i + hs, j : j + ws]))
                        for j in range(wr - ws)
                    ]
                    for i in range(hr - hs)
                ]
...
def find_bitmap(bmp, region, tol=0.01, mask=None, mode="dist"):
    ...
    for i in np.arange(0, hs):
        for j in np.arange(0, ws):
            if (mask is None and i == 0 and j == 0) or (
                mask is not None and not mask[i, j]
            ):
                continue
