...
def filter_near(a, b, dist):
    ...
    if len(a) == 0 or len(b) == 0:
        return np.asarray([])
    diffs = np.asarray(
        [(i, np.min(np.sum(np.square(p - b), axis=-1))) for i, p in enumerate(a)]
    )
...
def closest_shuffle(pt, pts, sigma):
    ...
    pts = np.asarray(pts)
    pt = np.asarray(pt)
    dists = np.sqrt(np.sum(np.square(pts - pt), axis=1)) + np.random.normal(
        0, sigma, len(pts)
    )
...
def cluster(points, radius=5):
    ...
    while True:  # loop until all points are clustered
        ...
        candidate = points[unclustered][
            np.random.randint(remaining)
